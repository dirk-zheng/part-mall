...
export default function AdminUsers() {
  ...
  useEffect(() => {
    adminAPI.getUsers().then(setUsers).catch((err) => setError(err.message)).finally(() => setLoading(false));
  }, []);
    ...
    <div className="min-h-screen bg-slate-50 pb-16 pt-24">
      <main className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <header className="mb-8">
          <p className="mb-2 text-sm font-semibold uppercase tracking-wider text-primary">Administrator</p>
        ...
        <label className="relative mb-6 block max-w-xl"><span className="sr-only">Search users</span><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={19} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, username, role or ID" className="w-full rounded-xl border-slate-300 py-3 pl-11" /></label>
