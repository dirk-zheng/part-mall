...
> **2026-08-19 B2B 询盘流程更新**：当前实现以 `/quote` 的 Mixed Load + RFQ 流程为准。产品卡使用 “Request Quote”，前台不展示零售库存与直接结账；询盘采集目标市场、车型、轮毂规格、预计数量、柜型及报告/清关要求，并生成持久化 RFQ 记录。下文旧版 Cart / Checkout 章节仅保留为历史设计参考。
